...
string_input_text_binary = ('0' * 3) + ('1' * 8)
...
# Реализация перебора
# list = ['b1', 'b2', '0', 'b4', '1', '0', '0']
# for pos_cor in [0, 1, 3]:
#     int_current_correction_bit = 0
#     for pos_tail_subbloc in range(pos_cor * 2, 7, 2 ** (pos_cor + 1)):
#         for pos_cur_subbloc_bit in range(pos_tail_subbloc - pos_cor, pos_tail_subbloc + 1):
#             if list[pos_cur_subbloc_bit][0] != 'b':
#                 int_current_correction_bit = int(ord(str(int_current_correction_bit)) ^
#                                                  ord(str(list[pos_cur_subbloc_bit])))
#     if int_current_correction_bit: int_current_correction_bit = 1
#     list[pos_cor] = str(int_current_correction_bit)
# print(list)

def hamming_codec_get_result(bool_encode_operation: bool) -> np.array:
    pass